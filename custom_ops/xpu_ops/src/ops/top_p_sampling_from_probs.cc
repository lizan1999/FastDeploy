...
std::vector<paddle::Tensor> TopPSamplingFromProbsKernel(
  ...
  paddle::Tensor output = paddle::empty(
      {batch_size}, paddle::DataType::INT32, probs.place());
  ...
  int ret = infer_ops::top_p_sampling_from_probs<float, int32_t>(
      ...
      topk);
  PD_CHECK(ret == 0,
           "top_p_sampling_from_probs_xpu: infer_ops call failed, ret=", ret);
