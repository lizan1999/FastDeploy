  ...
  if (FLAGS_encoder_splice && !rope_3d) {
    ...
    if (!use_neox_rotary_style) {
      ret = infer_ops::vsl_rotary_embedding_gptj<TQKV, TR, TID>(
          ...
          reinterpret_cast<const TR*>(rotary_embs.data()),
      ...
      // Generate positions [0, 1, ..., token_num-1] on XPU
      ...
      //               q_split.data(),
      //               token_num * q_num_heads * head_dim * sizeof(TQKV),
      //               XPU_DEVICE_TO_DEVICE);
      //   xpu_memcpy(const_cast<void*>(k.data()),
      //               k_split.data(),
      //               token_num * real_kv_num_heads * head_dim * sizeof(TQKV),
      //               XPU_DEVICE_TO_DEVICE);

      ret = infer_ops::rotary_embedding<TQKV, int64_t, TR>(
          xpu_ctx,
          positions_tensor.data<int64_t>(),
          const_cast<TQKV*>(reinterpret_cast<const TQKV*>(q.data())),
          const_cast<TQKV*>(reinterpret_cast<const TQKV*>(k.data())),
          reinterpret_cast<const TR*>(rotary_embs.data()),
          rope_head_dim * 2,             // rot_dim
          q_num_heads * head_dim,        // query_stride
          real_kv_num_heads * head_dim,  // key_stride
          head_dim,                      // head_stride
          q_num_heads,                   // num_heads
          real_kv_num_heads,             // num_kv_heads
          head_dim,                      // head_sizes
          token_num,                     // num_tokens
          rope_max_seqlen,               // max_position
          true);                         // is_neox
      PD_CHECK(ret == api::SUCCESS, "rotary_embedding failed.");
      //   xpu_ctx->set_debug_level(0x1);
    }
...
std::vector<paddle::Tensor> SplitEmbeddingKVCache(
  ...
  if (enc_batch > 0) {
    ...
    split_kvcache_encoder<XPU_XType, XPU_XType, XPU_CType, int, E_Scale>(
        xpu_ctx,
        xctx,
        qkv,
        rotary_embs,
  ...
  if (dec_batch > 0) {
    ...
    if (total_dec_len != dec_batch) {
      ...
      // non mtp branch
      split_kvcache_decoder<XPU_XType, XPU_XType, XPU_CType, int, D_Scale>(
          xpu_ctx,
          xctx,
          qkv,
          rotary_embs,
